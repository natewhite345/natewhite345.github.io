<?php
        ...
        $homewebsite='simpleanswers.ga';
        if(is_null($verifieralreadycalled)){include "verifier.php";}
        ...
        if(isset($_POST['commenttext'])){//this only triggers on reload following comment submission
                ...
                if(is_null($data)){
                        ...
                        foreach($data as $datum){//prevent spam
                                if($datum['content']==verify($_POST['commenttext'],"n",0)&&substr($datum['id'],0,-1)==substr($id,0,-1)){$additionalinfo.='<script>alert("The comment you submitted was a duplicate and will not be posted")</script>';$giveup=true;break;}
                        }    
                }
                if(!$giveup){
                $time=time();
                $newdata=['content'=>verify($_POST["commenttext"],"n",0),'submitted'=>$time,"posted"=>-1,"id"=>$id,"netvotes"=>0];
                if(isset($_POST['email'])&&$_POST['email']!=''){$newdata['email']=verify($_POST['email'],"n",0);};
                ...
                }else{$pageinfo=['title'=>"Cannot reach server",'content'=>'The website is temporarily down and the comment could not be posted. The site should be back online in under an hour. I apologize for the inconvenience.'];}
        }
        if($conn){
        ...
        }}else{$navbarhtml.="<p>Questions could not be reached</p>";}
        $navbarhtml.='</ul>';
    ...
    <div id="flexgroup">
            ...
            <div id="notaside">
                    ...
                    <main>
                        ...
                        <?php if(isset($pageinfo['p'])){echo "<div class='votesdiv' id='votesdiv'><span style='cursor:pointer' id='upvote".$pageinfo['p']."' onclick='upvote(\'".$pageinfo['p']."\')'>&#x21E7;</span><span>".$pageinfo['netvotes']."</span><span style='cursor:pointer' id='downvote".$pageinfo['p']."' onclick='downvote(\"".$pageinfo['p']."\")'>&#x21E9;</span></div>";}?>
                        </div>
                        <?php if(isset($pageinfo['submitted'])){date_default_timezone_set("America/New_York");echo "<h3>Submitted ".date('l, M d Y g:i:s A T',$pageinfo['submitted']).".<br />";}?><?php if(isset($pageinfo['posted'])){echo "Posted ".date('l, M d Y g:i:s A T',$pageinfo['posted']).".";echo "</h3>";}?>
...
if(isset($pageinfo['comments'])){
    ...
    if(isset($rawcomments)){
            ...
            function sortingbyvotes($a,$b){
                    if(!isset($a['netvotes'])||!isset($b['netvotes'])||$a['netvotes']==$b['netvotes']){return 0;}
                    else{return ($b['netvotes']>$a['netvotes'])?1:-1;}
            }
...
?>
     ...
     if(isset($pageinfo['p'])){echo '<script type="text/javascript">setarrows("'.$pageinfo['p'].'");</script>';};
     ?>
</body>
</html>
